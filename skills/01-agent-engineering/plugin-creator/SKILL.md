---
name: plugin-creator
description: Create and scaffold plugin directories for Codex with a required `.codex-plugin/plugin.json`, optional plugin folders/files, and baseline placeholders. ALWAYS use this skill when a user wants to create a Codex plugin, scaffold plugin structure, set up marketplace entries, or add plugin metadata—even if they don't explicitly say "plugin creator". Handles plugin name normalization, marketplace.json generation, and optional component folders (skills, hooks, scripts, assets, MCP, apps).
---
