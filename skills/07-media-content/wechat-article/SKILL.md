...
```bash
# 进入本 skill 所在目录（路径因安装位置而异，请替换为实际路径）
# 例如：cd ~/AI_Skills_Registry/Global/wechat-article
