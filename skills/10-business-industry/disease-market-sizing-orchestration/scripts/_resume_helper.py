"""Internal helper migrated from former `session-resume` skill (deleted 2026-04-27).

Responsible for:
- detect_partial_state: scan .cache/<slug>/ to infer last completed phase
- resume_from: human-readable resume message
- invalidate_dependents: precise cache invalidation when contract field changes

INVALIDATION_RULES preserves the 7-field cache invalidation policy from the
original skill (resume.py lines 83-110). Migrated in-place per architect review
af265bdc revision 5 — these are real business rules, not reactive patches.
"""
...
# Field → cache artifacts to clear when this contract field changes.
