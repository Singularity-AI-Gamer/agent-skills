...
## Missing implementation evidence does not cancel a contract

**Original:** “Acknowledged writes are not retried. The request deadline is five seconds, measured against a 3.2-second maximum observation.”

**Over-trimmed:** Delete both statements because the one inspected read helper contains no write or timeout logic.

**Balanced:** Keep the statements under the client-policy subject and report that their implementation owner was not established. Remove or revise them only after a current owner proves they were superseded.

A narrow implementation surface cannot disprove a broader documented contract. Treat missing ownership evidence as unknown rather than choosing code as the source of truth by convenience.
...
**Balanced:** “Mounted plugins share the host's authority; for example, access to the host command executor permits commands with the host process's privileges.”
...
**Balanced:** “Provider selection is cached for the plugin lifetime; installing or repairing a provider requires reload.” Keep ordinary cleanup in its TODO, issue, or owning proposal.
