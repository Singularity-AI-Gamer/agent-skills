---
name: seo
description: Conduct comprehensive SEO audits to identify technical blockers, on-page gaps, and content optimization opportunities. Audit crawlability (robots.txt, noindex, redirects), Core Web Vitals (LCP/INP/CLS), structured data schema markup, title/meta tag optimization, heading hierarchy, internal linking patterns, and keyword cannibalization. Use when the user wants search visibility improvements, wants to audit a site for SEO issues, needs help with schema validation or Core Web Vitals remediation, requests keyword mapping to URLs, or plans sitemap/robots.txt changes. Always prioritize technical fixes before content changes.
origin: ECC
---
